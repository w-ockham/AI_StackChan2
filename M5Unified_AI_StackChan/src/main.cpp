...
String json_ChatString = "{\"model\": \"gpt-3.5-turbo-1106\",\"messages\": [{\"role\": \"user\", \"content\": \""
                         "\"}]}";
String json_DifyString = "{\"inputs\": {},\"response_mode\": \"blocking\", \"conversation_id\" : \"\", \"user\": \"zunda-user\", \"query\" : \"\"}";
...
{
  String response = "";
  const char *api = "https://api.openai.com/v1/chat/completions";

  avatar.setExpression(Expression::Doubt);
  avatar.setSpeechText("考え中…");
  String ret = https_post_json(api, json_string.c_str(), root_ca_openai);
...
String difyAi(String json_string)
{
  String response = "";
  const char *api = "https://api.dify.ai/v1/chat-messages";

  avatar.setExpression(Expression::Doubt);
  avatar.setSpeechText("考え中…");
  Serial.println(json_string.c_str());
  String ret = https_post_json(api, json_string.c_str(), root_ca_difyai);
  avatar.setExpression(Expression::Neutral);
  avatar.setSpeechText("");
  Serial.print("Response=");
  Serial.println(ret);
...
{
  static String response = "";
  Serial.println(json_DifyString);
...
void exec_difyAi(String text)
{
  static String convId;
  static String response = "";

  init_chat_doc(json_DifyString.c_str());
  chat_doc["query"] = text;
  String json_string;
  serializeJson(chat_doc, json_string);
...
String SpeechToText(bool isGoogle)
{
  ...
  else
  {
    ...
    Serial.println("音声認識開始");
    avatar.setSpeechText("わかりました");
    Whisper *cloudSpeechClient = new Whisper(root_ca_openai, STT_API_KEY.c_str());
    ret = cloudSpeechClient->Transcribe(audio);
    delete cloudSpeechClient;
    delete audio;
  }
...
void loop()
{
  ...
  if (random_time >= 0 && millis() - lastms1 > random_time)
  {
    ...
    if (!mp3->isRunning() && speech_text == "" && speech_text_buffer == "")
    {
      ...
      exec_difyAi(random_words[random(18)]);
      mode = 0;
    }
  }
